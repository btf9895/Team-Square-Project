...
public class EdgeConvertGUI {
   ...
   static DefaultListModel<Object> dlmDRTablesRelations, dlmDRTablesRelatedTo, dlmDRFieldsTablesRelations,
   ...
   public EdgeConvertGUI() {
      logger.info("EdgeConvertGUI Constructer called");
   ...
   public void showGUI() {
      logger.info("showGUI method called");
      try {
         UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName()); // use the OS native LAF, as opposed to
                                                                              // default Java LAF
      } catch (Exception e) {
         logger.error("showGUI catch - Error setting native LAF: , e={}", e);
      }
      createDTScreen();
      createDRScreen();
      logger.info("showGUI method complete");
   } // showGUI()

   public void createDTScreen() {// create Define Tables screen
      logger.info("createDTScreen method called");
                     ...
                     switch (i) {
                        ...
                        case 2: // Integer
                           try {
                              ...
                              jtfDTDefaultValue.setText(intResult + "");
                              goodData = true;
                        ...
                        case 3: // Double
                           try {
                              ...
                              jtfDTDefaultValue.setText(doubleResult + "");
                              goodData = true;
                           } catch (NumberFormatException nfe) {
                              JOptionPane.showMessageDialog(null, "\"" + result
      ...
      jfDT.validate();
      logger.info("createDTScreen successful");

   } // createDTScreen

   public void createDRScreen() {
      logger.info("createDRScreen method called");
      ...
      logger.info("createDRScreen method complete");

   } // createDRScreen

   public static void setReadSuccess(boolean value) {
      logger.debug("setReadSuccess, value={}", value);
      readSuccess = value;
   }

   public static boolean getReadSuccess() {
      logger.debug("getReadSucces, value={}", readSuccess);
      return readSuccess;
   }
   ...
   private void saveAs() {
      ...
      logger.debug("saveAs successful, about to write to file");
      writeSave();
   }
   ...
   private void writeSave() {
      if (saveFile != null) {
         try {
            ...
            logger.error("writeSave catch of IOException, ioe={}", ioe);
         }
         dataSaved = true;
         logger.debug("write save successful");
      }
   }

   private void setOutputDir() {
      logger.info("setOutputDir called");
      int returnVal;
   ...
   private void getOutputClasses() {
      logger.info("getOutputClasses called");
      File[] resultFiles = {};
      ...
      if (classLocation.startsWith("jar:")) {
         ...
         logger.debug("Jarfile name: , jarfile={}", jarfilename);
         try (JarFile jarfile = new JarFile(jarfilename)) {
            ...
            logger.error("getOutputClasses catch of IOException, ioe={}", ioe);
            logger.warn("Throwing Runtime Exception");
      ...
      try {
         for (int i = 0; i < resultFiles.length; i++) {
            logger.info("Filename, filename={}", resultFiles[i].getName());
         ...
         logger.error("InstantiationException, ie={}", ie);
         ie.printStackTrace();
      } catch (ClassNotFoundException cnfe) {
         logger.error("ClassNotFoundException, cnfe={}", cnfe);
         cnfe.printStackTrace();
      } catch (IllegalAccessException iae) {
         logger.error("IllegalAccessException, iae={}", iae);
         iae.printStackTrace();
      } catch (NoSuchMethodException nsme) {
         logger.error("NoSuchMethodEception, nsme={}", nsme);
         nsme.printStackTrace();
      } catch (InvocationTargetException ite) {
         logger.error("InvocationTargetException, ite={}", ite);
         ite.printStackTrace();
      }
   ...
   private String getSQLStatements() {
      logger.info("getSQLStatements called");
      String strSQLString = "";
      ...
      try {
         ...
         logger.error("IlleglAccessException, iae={}", iae);
         iae.printStackTrace();
      } catch (NoSuchMethodException nsme) {
         logger.error("NoSuchMethodException, nsme={}", nsme);
         nsme.printStackTrace();
      } catch (InvocationTargetException ite) {
         logger.error("InvocationTargetException, ite={}", ite);
         ite.printStackTrace();
      }
      logger.info("getSQLStatements success");
      return strSQLString;
   }

   private void writeSQL(String output) {
      logger.info("writeSQL called");
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
         ...
         try {
            ...
            logger.debug("writeSQL output={}", output);
            pw.println(output);
            // close the file
            pw.close();
            logger.info("writeSQL success");
         } catch (IOException ioe) {
            logger.error("writeSQL catch of IOException, ioe={}", ioe);
         }
      }
   }
   ...
   class EdgeWindowListener implements WindowListener {
      ...
      public void windowClosing(WindowEvent we) {
         ...
         logger.info("No option was selected, exiting program");
   ...
   class CreateDDLButtonListener implements ActionListener {
      public void actionPerformed(ActionEvent ae) {
         ...
         logger.debug("CreateDDLButtonListener about to write, sqlString={}", sqlString);
         writeSQL(sqlString);
      }
   }
